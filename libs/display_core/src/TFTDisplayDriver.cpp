...
namespace
{
  static TFTDisplayDriverContext *ctx(void *context)
  {
    return static_cast<TFTDisplayDriverContext *>(context);
  }

  static void init(void *context, const DisplayConfig &config)
  {
    TFTDisplayDriverContext *c = ctx(context);
  ...
  {
    ctx(context)->sprite.fillSprite(color);
  }
  ...
  {
    ctx(context)->sprite.pushSprite(0, 0);
  }
...
DisplayDriverOps TFTDisplayDriver::makeOps()
{
...
DisplayConfig TFTDisplayDriver::makeConfig(TFTDisplayDriverContext *context, uint16_t width, uint16_t height, uint8_t rotation, uint8_t colorDepth)
{
